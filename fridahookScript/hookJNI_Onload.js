...
function hook_libart(){
	...
	for (var i=0;i<symbols.length;i++){
		...
		if(symbol.name=="ZN3art3JNI17GetStringUTFCharsEP7_JNIEnvP8_ jstringPh"){
			...
			console.log("GetFeildID is at",symbol.address,symbol.name);
			...
			console.log("addrRegisterNatives is at",symbol.address,symbol.name);
	...
	if(addrRegisterNatives!=null){
		Interceptor.attach(addrRegisterNatives,{
			onEnter:function(args){
				...
				var funcAllocOject=new NativeFunction(addrAllocObject,"pointer",["pointer","pointer"]);
				...
				var mid_getClass=funcGetMethodID(env,java_class,Memory.allocUtf8String("getClass"),Memory.
				allocUtf8String("()Ljava/lang/Class;"));
				var clz_obj2=funcCallOjectMethod(env,clz_obj,mid_getClass);
				var cls=funcGetObjectClass(env,clz_obj2);
				var mid_getName=funcGetMethodID(env,cls,Memory.allocUtf8String("getName"),Memory.allocUtf8String(
				"()Ljava/lang/Class;"));
				var name_jstring=funcCallObjectMethod(env,clz_obj2,mid_getClass);
				...
				for(var i=0;i<method_count;i++){
					...
					fnPtr_ptr,"module_name:",find_module.name,"module_base",find_module.base,"offset:",ptr(
